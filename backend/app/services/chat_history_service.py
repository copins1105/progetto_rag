...
"""
ChatHistoryService — FIX v2
============================

PROBLEMI CORRETTI:
  1. load_session_context: ora restituisce anche "is_new_session" flag
     così il chain sa se ha davvero un contesto conversazionale o no.

  2. salva_messaggio: i source_docs salvati in sources_json vengono
     deduplicati E filtrati: se la risposta è "not_found" o "courtesy"
     (nessun documento realmente usato), sources_json rimane vuoto.
     Questo evita che documenti "di contorno" del retriever contaminino
     il contesto storico e l'audit.

  3. load_session_context: aggiunto campo "n_messages_in_session" per
     permettere al chiamante di sapere se è davvero una nuova sessione
     (0 messaggi) o una sessione con storico reale.
"""
...
def load_session_context(session_uuid: str, utente_id: int) -> dict:
    ...
    FIX: aggiunto "is_new_session" nel risultato.
      - True  → sessione senza messaggi salvati (nuova o appena resettata)
      - False → sessione con storico reale

    Il chain usa questo flag per bloccare risposte "di cosa abbiamo parlato?"
    quando la sessione è effettivamente nuova.
    """
    from sqlalchemy import text
    from langchain_core.messages import HumanMessage, AIMessage
    ...
    empty = {
        ...
        "found":            False,
        "n_total":          0,
        "is_new_session":   True,   # FIX: nuovo campo
    }
    ...
    try:
        ...
        # FIX: sessione trovata ma senza messaggi → è una nuova sessione
        if not rows:
            return {
                **empty,
                "found":          True,
        ...
        return {
            ...
            "found":          True,
            "n_total":        n_total,
            "is_new_session": False,   # FIX: ha messaggi reali
        }
...
def salva_messaggio(
    ...
    try:
        ...
        # FIX: salva sources_json SOLO per risposte "content" (documenti realmente usati).
        # Per "courtesy", "not_found", "blocked" → lista vuota.
        # Questo evita che documenti del retriever "di contorno" inquinino
        # l'audit e il contesto delle sessioni successive.
        sources_json = []
        if tipo_risposta == "content" and source_docs:
            seen_keys: set = set()
        ...
        n_chunk = len(source_docs) if (tipo_risposta == "content" and source_docs) else 0
        ...
        logger.debug(
            f"[chat_history] salvato: sessione={session_uuid} "
            f"tipo={tipo_risposta} docs={documento_ids} sources={len(sources_json)}"
        )
...
# ─────────────────────────────────────────────
# RECUPERO: dettaglio singola sessione
# ─────────────────────────────────────────────
            ...
            "messaggi": [
                {
                    ...
                    "n_chunk":       r.n_chunk_recuperati,
                    "documento_ids": r.documento_ids or [],
                    # FIX: priorità a sources_json (dati salvati al momento),
                    # fallback a documenti_dettaglio (join live dal DB)
                    "fonti":         r.sources_json or r.documenti_dettaglio or [],
                    "documenti":     r.documenti_dettaglio or [],
                }
                for r in messaggi
            ],
