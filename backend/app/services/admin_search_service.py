...
Legge host/port/collection dalla pipeline attiva.
...
class AdminSearchService:
    def __init__(self):
        ...
        self.collection_name = ACTIVE_CONFIG.chroma_collection_name

        try:
            self.collection = self.client.get_collection(self.collection_name)
            logger.info(
                f"[{ACTIVE_CONFIG.pipeline.upper()}] AdminSearchService: {self.collection_name}"
            )
        except Exception as e:
            logger.error(f"AdminSearchService: impossibile connettersi: {e}")
            self.collection = None
    ...
    def _build_stem_map(self) -> None:
        ...
        try:
            ...
            for meta in metas:
                ...
                anchor = meta.get("anchor_link", "") or ""
                if anchor:
                    filename = anchor.split("/")[-1].split("#")[0]
            ...
            self._title_to_stems     = new_title_to_stems
            self._all_indexed_titles = new_all_titles
            logger.info(f"AdminSearchService: {len(new_stem_to_title)} stem, {len(new_all_titles)} titoli")
        except Exception as e:
            logger.warning(f"AdminSearchService build_stem_map: {e}")
    ...
    def is_indexed(self, stem: str) -> bool:
        ...
        try:
            ...
            logger.warning(f"AdminSearchService.is_indexed: {e}")
            return False
    ...
    def get_chunks(self, stem: str, page: int = 0, page_size: int = 15) -> dict:
        ...
        try:
            ...
            logger.error(f"AdminSearchService.get_chunks: {e}")
            return empty
    ...
    def delete_document(self, stem: str) -> int:
        ...
        try:
            ...
            logger.error(f"AdminSearchService.delete_document: {e}")
            return 0