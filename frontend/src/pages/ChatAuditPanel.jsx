...
// // FIX LINK CLICCABILI v2:
// // - DocBadge usa il campo "fonti" (sources_json salvato al momento della risposta)
// //   che contiene { titolo, link, pagina } già risolti.
// // - Se "fonti" è vuoto ma ci sono "documenti" (join live), usa quelli come fallback.
// // - buildDocHref: se link è un path relativo /static/... lo prefissa con API_BASE.
// //   Se link manca, costruisce l'URL dal titolo.
...
// // FIX: costruisce href da fonti salvate in sources_json.
// // Il campo "fonti" arriva come array di { titolo, link, pagina }.
// function buildDocHref(doc) {
//   // Normalizza i nomi dei campi (fonti usa "titolo"/"pagina", documenti usa "title"/"page")
//   const titolo = doc.titolo || doc.title || "";
//   const link   = doc.link   || "";
//   const pagina = doc.pagina ?? doc.page ?? null;
...
//       base = link;
//     } else {
//       // Path relativo → prefissa con API_BASE
//       base = `${API_BASE}${link.startsWith("/") ? "" : "/"}${link}`;
//     }
//     // Rimuovi eventuale #page già presente prima di aggiungerne uno nuovo
//     base = base.replace(/#page=\d+$/, "");
//   } else if (titolo) {
//     // Nessun link salvato → costruisci URL dal titolo
//     base = `${API_BASE}/api/v1/admin/pdf/${encodeURIComponent(titolo)}`;
//   }

//   if (!base) return null;

//   // Aggiungi numero pagina come anchor
//   if (pagina && String(pagina).trim() && String(pagina).trim() !== "N/D") {
//     return `${base}#page=${pagina}`;
//   }
//   return base;
// }

// // Restituisce label leggibile per il badge documento
// function buildDocLabel(doc) {
//   const titolo = doc.titolo || doc.title || "—";
//   const pagina = doc.pagina ?? doc.page ?? null;
//   const label  = pagina && String(pagina).trim() && String(pagina).trim() !== "N/D"
//     ? `📄 ${titolo} · p.${pagina}`
//     : `📄 ${titolo}`;
//   return label;
// }
...
// // ─── DocBadge — FIX: usa buildDocHref aggiornato ──────────────────────────────
...
//   // FIX: usa "fonti" (sources_json) come fonte primaria,
//   // fallback a "documenti" (join live) se fonti è vuoto.
//   const fonti = (msg.fonti && msg.fonti.length > 0) ? msg.fonti
//               : (msg.documenti || []);
...
//       {/* FIX: mostra fonti solo se ci sono (tipo "content") */}
//       {fonti.length > 0 && (
//         <div style={s.docsRow}>
//           <span style={{ fontSize: "0.65rem", color: "var(--text-muted)", flexShrink: 0, marginRight: 2 }}>
//             Fonti:
//           </span>
//           {fonti.map((d, i) => <DocBadge key={i} doc={d} />)}
//         </div>
//       )}
...
// src/pages/ChatAuditPanel.jsx — RESPONSIVE
// Mobile: list and detail stack vertically (50/50 split)
// Tablet: narrower list (260px), detail fills rest
// Fixed: overlapping title/delete button, doc badges wrapping correctly
...
const sessDate = {
  fontSize: "0.65rem", color: "var(--text-muted)", fontFamily: "'JetBrains Mono', monospace",
};
  ...
  if (href) {
    return (
      <a href={href} target="_blank" rel="noreferrer"
        ...
        onMouseEnter={e => { e.currentTarget.style.background = "rgba(53,128,184,0.28)"; }}
        onMouseLeave={e => { e.currentTarget.style.background = "var(--accent-dim)"; }}>
        {label}
      </a>
    );
  }
  return <span style={{ ...baseStyle, opacity: 0.65, cursor: "default" }}>{label}</span>;
}
...
function MsgBlock({ msg, idx }) {
  ...
  return (
    <div style={{
      ...
      <div
        style={{ padding: "10px 14px", fontSize: "0.8rem", color: "var(--text)", cursor: "pointer" }}
        onClick={() => setOpen(o => !o)}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 4, gap: 8 }}>
          <span style={{ fontSize: "0.7rem", color: "var(--text-muted)" }}>
            Risposta {open ? "▲" : "▼"}
          </span>
          <span style={sessBadge(tipoColor)}>{msg.tipo_risposta}</span>
        </div>
        {open ? (
          <div style={{ fontSize: "0.78rem", lineHeight: 1.6 }}>
            <ReactMarkdown remarkPlugins={[remarkGfm]}>{msg.risposta || "—"}</ReactMarkdown>
      ...
      <div style={{
        ...
        <span style={sessDate}>{fmtDate(msg.timestamp)}</span>
        {msg.latency_ms && (
          <span style={sessBadge({ background: "var(--surface2)", color: "var(--text-muted)", border: "1px solid var(--border)" })}>
            {msg.latency_ms}ms
          </span>
        )}
        {msg.n_chunk > 0 && (
          <span style={sessBadge({ background: "rgba(53,128,184,0.08)", color: "var(--accent)", border: "1px solid rgba(53,128,184,0.2)" })}>
            {msg.n_chunk} chunk
          </span>
        )}
        {msg.bloccato && <span style={sessBadge(TIPO_COLORS.blocked)}>BLOCCATO</span>}
        {msg.feedback_csat && (
          <span style={{ fontSize: "0.9rem" }} title={`CSAT: ${msg.feedback_csat}/5`}>
            {CSAT_LABELS[msg.feedback_csat]}
          </span>
...
function SessionDetail({ session_uuid, onDelete, isSuperAdmin, onBack }) {
  const { authFetch } = useAuth();
  const [detail, setDetail]   = useState(null);
  const [loading, setLoading] = useState(false);
  const [confirm, setConfirm] = useState(false);
  ...
  if (loading) {
    return (
      <div style={{
        ...
        height: "100%", color: "var(--text-muted)", fontSize: "0.78rem",
      }}>Caricamento…</div>
    );
  }
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%", overflow: "hidden" }}>
      ...
      <div style={{
        ...
        {/* Back button (mobile only) + title + delete — no overlap */}
        <div style={{ display: "flex", alignItems: "flex-start", gap: 8, marginBottom: 6 }}>
          ...
          <div style={{
            ...
            flex: 1, minWidth: 0,
            overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap",
          }}>{detail.titolo || "Conversazione"}</div>
          {isSuperAdmin && (
              ...
              {confirm ? (
                <div style={{ display: "flex", gap: 4 }}>
                  <button onClick={handleDelete} style={{
                    ...
                    border: "1px solid rgba(239,68,68,0.35)", borderRadius: 6,
                    color: "#f87171", cursor: "pointer", fontFamily: "inherit", fontSize: "0.7rem",
                    whiteSpace: "nowrap",
                  }}>Conferma</button>
                  <button onClick={() => setConfirm(false)} style={{
        ...
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
          <span style={{ fontSize: "0.72rem", color: "var(--text-muted)" }}>👤 {nomeUtente}</span>
          <span style={sessDate}>{fmtDate(detail.creata_il)}</span>
          <span style={sessBadge({ background: "var(--surface2)", color: "var(--text-muted)", border: "1px solid var(--border)" })}>
            {(detail.messaggi || []).length} msg
          </span>
          {detail.durata_secondi && (
            <span style={sessBadge({ background: "var(--surface2)", color: "var(--text-muted)", border: "1px solid var(--border)" })}>
              {fmtDurata(detail.durata_secondi)}
          ...
          <span style={{ fontSize: "0.6rem", color: "var(--text-muted)", fontFamily: "'JetBrains Mono', monospace", wordBreak: "break-all" }}>
            {detail.session_uuid}
          </span>
        </div>
      </div>
  ...
  return (
    <div
      ...
      <div style={{ display: "flex", gap: 6, marginTop: 3, flexWrap: "wrap", alignItems: "center" }}>
        <span style={sessDate}>{fmtDate(sess.aggiornata_il)}</span>
        <span style={sessBadge({ background: "var(--surface2)", color: "var(--text-muted)", border: "1px solid var(--border)" })}>
          {sess.n_messaggi || sess.n_log_risposta || 0} msg
        </span>
        {sess.n_documenti_unici > 0 && (
          <span style={sessBadge({ background: "rgba(53,128,184,0.1)", color: "var(--accent)", border: "1px solid rgba(53,128,184,0.25)" })}>
            {sess.n_documenti_unici} doc
          </span>
        )}
        {sess.n_bloccati > 0 && (
          <span style={sessBadge(TIPO_COLORS.blocked)}>bloccato</span>
        )}
      </div>
    </div>
  );
    ...
    <div style={{
      ...
      <div style={{ padding: "12px 14px", borderBottom: "1px solid var(--border)", flexShrink: 0 }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
          {isSuperAdmin && (
            <input style={inputStyle} placeholder="🔍 Utente…"
              value={fUtente} onChange={e => setFUtente(e.target.value)}
              onKeyDown={e => e.key === "Enter" && fetchSessioni(0)} />
          )}
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6 }}>
            <div>
              <div style={{ fontSize: "0.62rem", color: "var(--text-muted)", marginBottom: 3 }}>Da</div>
              <input type="date" style={inputStyle} value={fDataDa} onChange={e => setFDataDa(e.target.value)} />
            </div>
            <div>
              <div style={{ fontSize: "0.62rem", color: "var(--text-muted)", marginBottom: 3 }}>A</div>
              <input type="date" style={inputStyle} value={fDataA} onChange={e => setFDataA(e.target.value)} />
            </div>
          </div>
          <label style={{ display: "flex", alignItems: "center", gap: 6, fontSize: "0.72rem", color: "var(--text-muted)", cursor: "pointer" }}>
            <input type="checkbox" checked={fSoloBloccate}
              onChange={e => setFSoloBloccate(e.target.checked)} />
            Solo sessioni con messaggi bloccati
          </label>
          <div style={{ display: "flex", gap: 6 }}>
            <button
      ...
      {/* Session list */}
      <div style={{ flex: 1, minHeight: 0, overflowY: "auto", overflowX: "hidden", padding: "6px 8px" }}>
        {loading && <div style={{ padding: 20, textAlign: "center", color: "var(--text-muted)", fontSize: "0.78rem" }}>Caricamento…</div>}
        ...
        {!loading && sessioni.map(sess => (
          <SessRow key={sess.session_uuid} sess={sess} selected={selected} onClick={() => handleSelect(sess)} />
        ))}
      </div>
      ...
      {totalPages > 1 && (
        <div style={{
          ...
          <button style={{ ...btnGhost, padding: "4px 8px" }} disabled={page <= 0} onClick={() => fetchSessioni(page - 1)}>←</button>
          <span style={{ fontSize: "0.7rem", color: "var(--text-muted)", fontFamily: "'JetBrains Mono', monospace" }}>
            {page + 1} / {totalPages}
          </span>
          <button style={{ ...btnGhost, padding: "4px 8px" }} disabled={page >= totalPages - 1} onClick={() => fetchSessioni(page + 1)}>→</button>
        </div>
      )}
    </div>
  );
