...
// RESPONSIVE UPDATE:
// - Hamburger button on mobile to toggle sidebar drawer
// - Sidebar backdrop overlay when open on mobile
// - Uses responsive.css classes + JS state for open/close
...
const BASE_SERVER_URL = import.meta.env.VITE_API_URL || 'https://127.0.0.1:8080'
...
function DebugDrawer({ open, onClose, debugData }) {
  if (!open) return null
  return (
    <div className="debug-drawer" style={{ position:'fixed', top:0, right:0, bottom:0, width:420,
      background:'var(--surface)', borderLeft:'1px solid var(--border-strong)',
      display:'flex', flexDirection:'column', zIndex:200,
      boxShadow:'-8px 0 36px rgba(0,0,0,0.45)' }}>
      <div style={{ padding:'16px 18px', borderBottom:'1px solid var(--border)',
...
export default function ChatPage() {
  ...
  // Close sidebar on navigation (e.g. when session selected on mobile)
  ...
  const handleResetChat = useCallback(async () => {
    ...
    setLogIds({})
    closeSidebar()
  }, [resetChat, sessionId, authFetch, closeSidebar])
  ...
  const handleSelectSession = useCallback(async (sess) => {
    if (sess.session_uuid === sessionId) { closeSidebar(); return }
    try {
      const res  = await authFetch(`/api/v1/chat/sessions/${sess.session_uuid}`)
      const data = await res.json()
      const INITIAL_MSG = {
      ...
      for (const m of (data.messaggi || [])) {
        msgs.push({ role: 'user', text: m.domanda })
        const rawSources = m.fonti || m.documenti || []
        msgs.push({ role: 'bot', text: m.risposta, sources: normalizeSources(rawSources), isHistorical: true })
      }
      if (data.messaggi && data.messaggi.length > 0) {
        msgs.push({
          role: 'bot',
  ...
  }, [authFetch, sessionId, loadSession, closeSidebar])
    ...
    <div className="app-container">
      ...
      <aside className={`chat-sidebar${sidebarOpen ? ' sidebar-open' : ''}`}>
        ...
        <div className="sidebar-footer">
          ...
          {lastDebug && (
            <button onClick={() => { setDebugOpen(true); closeSidebar() }} style={{
              width:'100%', padding:'7px 10px', marginBottom:6,
              background:'var(--accent-dim)', border:'1px solid var(--border-accent)',
              borderRadius:'var(--radius-sm)', color:'var(--accent-bright)',
              fontFamily:'inherit', fontSize:'0.78rem', fontWeight:600, cursor:'pointer' }}>
          ...
          <button className="admin-nav-btn" onClick={() => { navigate('/profile'); closeSidebar() }} style={{ marginBottom:6 }}>
            <span>👤</span>
            {user?.nome ? `${user.nome} ${user.cognome || ''}`.trim() : user?.email}
          </button>

          {hasPermission('page_admin') && (
            <button className="admin-nav-btn" onClick={() => { navigate('/admin'); closeSidebar() }} style={{ marginBottom:6 }}>
              <span>⚙</span> Pannello Admin
            </button>
          )}

          <button className="admin-nav-btn" onClick={handleLogout}
            style={{ color:'var(--red)', borderColor:'var(--red-dim)', marginBottom:6 }}>
            <span>↩</span> Esci
          </button>
      ...
      <main className="chat-window">
        ...
        <div className="chat-topbar" style={{ position:'relative', zIndex:1 }}>
          ...
          <div style={{ display:'flex', alignItems:'center', gap:12 }}>
            ...
            <div className="online-indicator">
              <div className="online-dot" />
              <span style={{ display: 'var(--topbar-status-display, inline)' }}>Connesso</span>
            </div>
          </div>
        </div>
