...
// src/pages/AdminPanel.jsx — RESPONSIVE UPDATE
// Added className="admin-panel-body/sidebar/viewer/right" so CSS media queries
// can reflow the 3-column layout on mobile/tablet.
// Also added className="admin-toolbar", "admin-upload-zone", "admin-pdf-item",
// "admin-right-tabs", "admin-log-content", "admin-toggle-btn".
//
// All logic and functionality is identical to the original.
...
const s = {
  root: { display: "flex", flexDirection: "column", height: "100%", width: "100%", background: "var(--bg)", color: "var(--text)", fontFamily: "'DM Sans', sans-serif", overflow: "hidden" },
  toolbar: { display: "flex", alignItems: "center", gap: 8, padding: "8px 16px", background: "var(--surface)", borderBottom: "1px solid var(--border)", flexShrink: 0, flexWrap: "wrap" },
  body: { display: "flex", flex: 1, overflow: "hidden", minHeight: 0 },
  sidebar: { width: "280px", flexShrink: 0, display: "flex", flexDirection: "column", background: "var(--surface)", borderRight: "1px solid var(--border)", overflow: "hidden", height: "100%", minHeight: 0 },
  sidebarHeader: { display: "flex", alignItems: "center", justifyContent: "space-between", padding: "14px 16px 10px", borderBottom: "1px solid var(--border)", flexShrink: 0 },
  sidebarTitle: { fontSize: "0.88rem", fontWeight: 600, color: "var(--text)" },
  ...
  syncDot: (stato) => { const dots = { synced: "#34d399", solo_postgres: "#fbbf24", solo_chroma: "#fbbf24", mismatch: "#f87171", error: "#f87171", pending: "#fbbf24" }; return { width: 7, height: 7, borderRadius: "50%", background: dots[stato] || "var(--text-muted)", flexShrink: 0, marginTop: 3 } },
  overlay: { position: "fixed", inset: 0, background: "rgba(0,0,0,0.6)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000 },
  dialog: { background: "var(--surface)", border: "1px solid var(--border-strong)", borderRadius: 14, padding: "28px 28px 24px", width: 360, maxWidth: "calc(100vw - 32px)", boxShadow: "0 24px 48px rgba(0,0,0,0.5)" },
  dialogTitle: { fontSize: "0.95rem", fontWeight: 600, color: "var(--text)", marginBottom: 8 },
  dialogText: { fontSize: "0.8rem", color: "var(--text-muted)", lineHeight: 1.6, marginBottom: 20 },
  dialogBtns: { display: "flex", gap: 8, justifyContent: "flex-end" },
  dialogCancel: { padding: "8px 16px", background: "none", border: "1px solid var(--border-strong)", borderRadius: 8, color: "var(--text-muted)", cursor: "pointer", fontFamily: "inherit", fontSize: "0.82rem" },
...
function Sidebar({ pdfs, selected, onSelect, onUploaded, onRefresh }) {
  return (
    <aside className="admin-panel-sidebar" style={s.sidebar}>
      <div style={s.sidebarHeader}>
        <div>
          <div style={s.sidebarTitle}>Knowledge Base</div>
          <div style={s.sidebarCount}>{pdfs.length} document{pdfs.length !== 1 ? "i" : "o"}</div>
      ...
      <div style={s.pdfList}>
        ...
        {pdfs.map((pdf) => {
          const sel = selected?.filename === pdf.filename
          return (
            <button key={pdf.filename} className="admin-pdf-item" style={s.pdfItem(sel)} onClick={() => onSelect(pdf)}>
              <div style={s.pdfRow}>
                <div style={{ minWidth: 0, flex: 1 }}>
                  <span style={s.pdfName(sel)}>{pdf.filename}</span>
                  <div style={s.pdfSize}>{pdf.size_kb} KB</div>
...
function PdfViewer({ filename, activeChunk, onClearChunk }) {
  ...
  return (
    <div className="admin-panel-viewer" style={s.viewer}>
      <div style={s.viewerToolbar}>
        <span style={s.viewerFilename}>{filename}</span>
        <div style={s.viewerControls}>
          <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
  ...
  return (
    <div>
      ...
      {logs.length > 0 && (
        <div style={s.logBox}>
          <div style={s.logHeader}><div style={s.logDot(status === "processing")} />log output</div>
          <div className="admin-log-content" style={s.logContent}>{logs.map((line, i) => <div key={i} style={s.logLine}>{line}</div>)}<div ref={logsEndRef} /></div>
        </div>
      )}
    </div>
  )
...
function ChunkExplorer({ filename, onChunkSelect, activeChunkId }) {
  ...
  return (
    <div className="admin-chunk-explorer">
      <div style={s.chunkStats}>
        <span>{total} chunk indicizzati</span>
        <span style={{ fontFamily: "'DM Mono', monospace", fontSize: "0.68rem" }}>pag. {page + 1}/{Math.max(totalPages, 1)}</span>
      </div>
...
function SyncPanel() {
  ...
  return (
    <div className="sync-panel-toolbar" style={{ overflow: "auto" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12, gap: 8, flexWrap: "wrap" }}>
        <span style={{ fontSize: "0.75rem", color: "var(--text-muted)" }}>
          {filtered.length}{filtered.length !== docs.length ? ` / ${docs.length}` : ""} documenti monitorati
        </span>
        <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
          {isSuperAdmin && (
            <input
              ...
              value={filterText}
              onChange={e => setFilterText(e.target.value)}
              className="sync-panel-filter-input"
              style={{
                padding: "5px 10px", background: "var(--surface2)",
                border: "1px solid var(--border-strong)", borderRadius: 6,
                color: "var(--text)", fontFamily: "inherit", fontSize: "0.75rem",
          ...
          <button style={{ background: "none", border: "none", cursor: "pointer", fontSize: "0.75rem", color: "var(--accent)" }} onClick={fetch_}>
            ↻ Aggiorna
          </button>
        </div>
      </div>
          ...
          <div key={i} style={s.syncItem(doc.stato)}>
            <div style={{ display: "flex", gap: 8, alignItems: "flex-start" }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                {isSuperAdmin && (
                  <div style={{ display: "inline-flex", alignItems: "center", gap: 5, marginTop: 5, padding: "2px 8px", borderRadius: 20, background: owner ? "rgba(139,92,246,0.10)" : "rgba(107,114,128,0.08)", border: owner ? "1px solid rgba(139,92,246,0.25)" : "1px solid var(--border)" }}>
                    <span style={{ fontSize: "0.6rem", color: "var(--text-muted)", fontStyle: "italic" }}>caricato da</span>
                    {owner ? (
                      <>
                        <span style={{ width: 16, height: 16, borderRadius: "50%", background: "rgba(139,92,246,0.2)", border: "1px solid rgba(139,92,246,0.35)", display: "inline-flex", alignItems: "center", justifyContent: "center", fontSize: "0.5rem", fontWeight: 700, color: "#a78bfa", flexShrink: 0, fontFamily: "'JetBrains Mono', monospace" }}>
                          {owner.nome.slice(0, 2).toUpperCase()}
                        </span>
                        <span style={{ fontSize: "0.65rem", fontWeight: 600, color: "#a78bfa", maxWidth: 160, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }} title={owner.email}>{owner.nome}</span>
                      </>
                    ) : (
                      <span style={{ fontSize: "0.65rem", color: "var(--text-muted)", fontStyle: "italic" }}>Sistema / SuperAdmin</span>
                    )}
                  </div>
                )}
              </div>
...
function RightPanel({ pdf, onStatusChange, onDeleted, onRefresh, onChunkSelect, activeChunkId }) {
  ...
  const handleDelete = useCallback(async () => {
    ...
    try {
      const res  = await authFetch(`/api/v1/admin/document/${encodeURIComponent(pdf.filename)}`, { method: "DELETE" })
      const data = await res.json()
      setShowDelete(false); setDeleteResult(data)
      if (onDeleted) onDeleted(pdf.filename)
    } catch (e) { setDeleteResult({ ok: false, errors: [e.message], removed: [] }) }
    finally { setDeleting(false) }
  }, [authFetch, pdf?.filename, onDeleted])

  if (!pdf) {
    return (
      <div className="admin-panel-right" style={s.right}>
        <div style={s.rightEmpty}>
          <svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1" strokeLinecap="round" strokeLinejoin="round" style={{ opacity: 0.25 }}><path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z"/><polyline points="14 2 14 8 20 8"/></svg>
          <p style={{ fontSize: "0.78rem", textAlign: "center", lineHeight: 1.6 }}>Seleziona un documento per vederne i dettagli</p>
        </div>
  ...
  return (
    <div className="admin-panel-right" style={s.right}>
      ...
      <div style={s.rightHeader}>
        ...
        {deleteResult && !deleteResult.ok && (
          <div style={{ ...s.errorBanner, marginTop: 8, fontSize: "0.72rem" }}>⚠️ Eliminazione parziale. Errori: {deleteResult.errors?.join(", ")}</div>
        )}
      </div>

      <div className="admin-right-tabs" style={s.tabs}>
        {allTabs.map(t => (
          <button key={t.id} style={{ ...s.tab(currentTab === t.id), opacity: t.disabled ? 0.35 : 1 }} disabled={t.disabled} onClick={() => !t.disabled && setActiveTab(t.id)}>
            {t.label}
          </button>
      ...
      <div style={s.rightBody}>
        {currentTab === "ingest" && (
          <IngestionPanel pdf={pdf} onIngested={() => onStatusChange(pdf.filename, "ready")} onStatusChange={(ns) => onStatusChange(pdf.filename, ns)} />
        )}
        {currentTab === "loader" && (
          <LoaderPanel pdf={pdf} onLoaded={() => { onStatusChange(pdf.filename, "completed"); onRefresh() }} onStatusChange={(ns) => onStatusChange(pdf.filename, ns)} />
        )}
        {currentTab === "chunks"   && pdf.status === "completed" && <ChunkExplorer filename={pdf.filename} onChunkSelect={onChunkSelect} activeChunkId={activeChunkId} />}
        {currentTab === "modifica" && pdf.status === "completed" && <EditPanel pdf={pdf} onUpdated={() => {}} />}
        {currentTab === "sync"     && <SyncPanel />}
...
export default function AdminPanel() {
  ...
  const [activeChunk, setActiveChunk] = useState(null)
  const pollingRef     = useRef(null)
  const fetchPdfsRef   = useRef(null)
  const jobsRef        = useRef(jobs)
  const loaderJobsRef  = useRef(loaderJobs)
  ...
  const fetchPdfs = useCallback(async () => {
    try {
      ...
      const newPdfs = data.pdfs || []
      setPdfs(prev => newPdfs.map(serverPdf => {
        const hasActiveJob = jobsRef.current[serverPdf.filename]?.status === "processing" || loaderJobsRef.current[serverPdf.filename]?.status === "processing"
        const localStatus  = prev.find(p => p.filename === serverPdf.filename)?.status
        if (hasActiveJob && localStatus === "processing") return { ...serverPdf, status: "processing" }
        return serverPdf
      }))
      setSelected(prev => {
        if (!prev) return prev
      ...
      return newPdfs
    } catch (e) { console.error("Fetch pdfs error:", e); return [] }
  }, [authFetch])
  ...
  useEffect(() => {
    const hasPdfProcessing = pdfs.some(p => p.status === "processing")
    const hasJobProcessing = Object.values(jobsRef.current).some(j => j.status === "processing") || Object.values(loaderJobsRef.current).some(j => j.status === "processing")
    const shouldPoll = hasPdfProcessing || hasJobProcessing
    if (shouldPoll && !pollingRef.current) {
      pollingRef.current = setInterval(async () => {
        const fresh = await fetchPdfsRef.current?.()
        if (!fresh) return
        const stillHasProcessing = fresh.some(p => p.status === "processing") || Object.values(jobsRef.current).some(j => j.status === "processing") || Object.values(loaderJobsRef.current).some(j => j.status === "processing")
        if (!stillHasProcessing) { clearInterval(pollingRef.current); pollingRef.current = null }
      }, 3000)
    } else if (!shouldPoll && pollingRef.current) {
  ...
  const handleSelect = useCallback((pdf) => { setSelected(pdf); setActiveChunk(null) }, [])

  return (
    <div style={s.root}>
      {/* Toolbar */}
      <div className="admin-toolbar" style={s.toolbar}>
        <ToggleBtn active={leftOpen} onClick={() => setLeftOpen(o => !o)} label="Lista documenti" />
        <ToggleBtn active={rightOpen} onClick={() => setRightOpen(o => !o)} label="Dettagli" />
        {activeChunk && (
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginLeft: "auto", padding: "4px 12px", borderRadius: 6, background: "rgba(79,142,247,0.08)", border: "1px solid rgba(79,142,247,0.2)" }}>
      ...
      {/* Body — 3-column layout with responsive classNames */}
      <div className="admin-panel-body" style={s.body}>
        {leftOpen && (
          <Sidebar
            pdfs={pdfs}
            selected={selected}
...
function ToggleBtn({ active, onClick, label }) {
  return (
    <button className="admin-toggle-btn" onClick={onClick} style={{ padding: "5px 12px", borderRadius: 6, border: "1px solid var(--border-strong)", background: active ? "var(--accent-dim)" : "none", color: active ? "var(--accent)" : "var(--text-muted)", cursor: "pointer", fontFamily: "inherit", fontSize: "0.75rem", fontWeight: 600, display: "flex", alignItems: "center", gap: 6, transition: "all 0.15s" }}>
      <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
        <rect x="3" y="3" width="18" height="18" rx="2"/><line x1="9" y1="3" x2="9" y2="21"/>
      </svg>
      {label}
